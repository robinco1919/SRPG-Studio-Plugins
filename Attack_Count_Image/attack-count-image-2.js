...
IMPORTANT: Only use this plugin if you are also using no-counter-pursuit.js!
If you aren't then use attack-count-image-1 instead.
...
This plugin is compatible with no-counter-pursuit.js. If this plugin is 
being used and a unit cannot follow-up due to not initiating the attack, 
the window will displayed a crossed out icon.
...
StatusRenderer.drawAttackStatus = function(x, y, arr, color, font, space, count, roundCountReduced) {
	var i, text;
	...
	// The image is shown if count > 1 or count = 1 and roundCountReduced is true
	// (i.e. would cormally double but can't due to not initiating the attack)
	var displayPic = (count > 1 || (count == 1 && roundCountReduced));
	...
	if (displayPic && pic !== null) {
		var row = count - 1;
		var col = roundCountReduced;
		x += 18 + numberSpace;
		if (arr[0] >= 0) {
			NumberRenderer.drawNumber(x, y, arr[0]);
			pic.drawParts(x + 12, y + 6, col * width, row * height, width, height);
		}
		else {
...
var PosAttackWindow = defineObject(PosBaseWindow,
{
	...
	setPosTarget: function(unit, item, targetUnit, targetItem, isSrc) {
		...
		if (isCalculation) {
			...
			// New line here
			this._roundCountReduced = Calculator.roundCountReduced(unit, targetUnit, item, isSrc);
		}
		else {
	...
	drawInfoBottom: function(xBase, yBase) {
		...
		StatusRenderer.drawAttackStatus(x, y, this._statusArray, color, font, 20, this._roundAttackCount, this._roundCountReduced);
	}
}
);
...
UIBattleLayout._getroundCountReduced = function(unit, targetUnit, isSrc) {
	var roundAttackCount, isCounterattack;
	
	if (isSrc) {
		roundAttackCount = Calculator.roundCountReduced(unit, targetUnit,  BattlerChecker.getRealBattleWeapon(unit));
	}
	else {
		isCounterattack = this._realBattle.getAttackInfo().isCounterattack;
		if (isCounterattack) {
			roundAttackCount = Calculator.roundCountReduced(targetUnit, unit,  BattlerChecker.getRealBattleWeapon(targetUnit));
		}
		else {
			roundAttackCount = false;
		}
	}
...
UIBattleLayout.setBattlerAndParent = function (battlerRight, battlerLeft, realBattle) {
	...
	this._roundCountReducedLeft = this._getroundCountReduced(unit, targetUnit, battlerLeft.isSrc());
	this._roundCountReducedRight = this._getroundCountReduced(unit, targetUnit, battlerRight.isSrc());
...
UIBattleLayout._drawInfoArea = function(unit, isRight) {
	...
	if (isRight) {
		...
		roundCountReduced = this._roundCountReducedRight;
	}
	else {
		x = dx;
		arr = this._statusLeft;
		count = this._attackCountLeft;
		roundCountReduced = this._roundCountReducedLeft;
	}
	
	y = 65;
	StatusRenderer.drawAttackStatus(x, y, arr, color, font, 15, count, roundCountReduced);
};
=======
/*------------------------------------------------------------------------------
Attack Count Image
...
>>>>>>> c4d049b6189dee8359b1150905a6c985ed5ddc50
